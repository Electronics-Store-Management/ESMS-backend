package com.penguin.esms.components.supplier.viewDiscontinued;
...
class ViewDisSupplierTestRunner {
    ...
    @Autowired
    public ViewDisSupplierTestRunner(MockMvc mockMvc, ObjectMapper objectMapper, SupplierRepo supplierRepo, StaffRepository staffRepository, TestService testService) {
    ...
    public static List<TestCase> testData() throws IOException {
        return TestUtils.readTestDataFromCsv("src\\test\\java\\com\\penguin\\esms\\components\\supplier\\viewDiscontinued\\test-cases.csv", new ArrayList<>(List.of("name")), new ArrayList<>(List.of("length", "status")));
    }
