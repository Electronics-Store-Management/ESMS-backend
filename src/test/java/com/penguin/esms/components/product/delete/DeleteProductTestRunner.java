package com.penguin.esms.components.product.delete;
...
class DeleteProductTestRunner {
    ...
    @Autowired
    public DeleteProductTestRunner(MockMvc mockMvc, ObjectMapper objectMapper, ProductRepo productRepo, StaffRepository staffRepository, TestService testService) {
    ...
    public void shouldNotDeleteProduct() throws Exception {
        newProduct();
        mockMvc.perform(MockMvcRequestBuilders.delete("/product/dfiu" + product.getId())
                        .contentType(MediaType.MULTIPART_FORM_DATA_VALUE)
                        .header("Authorization", "Bearer " + authenticationResponse.getAccessToken())
                )
    ...
    @Test
    public void shouldNotDeleteDiscontinuedProduct() throws Exception {
    ...
    public  void newDiscontinuedProduct() throws Exception{
        ProductEntity neew = new ProductEntity();
        neew.setName(testService.generateRandomString(testService.ALL_CHARACTERS, 10));
        neew.setQuantity(20);
        neew.setPhotoURL(testService.generateRandomString(testService.ALL_CHARACTERS, 15));
